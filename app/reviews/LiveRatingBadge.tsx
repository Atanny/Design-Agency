  ...
  return (
    <div className="flex items-center justify-center gap-3 mt-4">
      <div className="flex gap-1">
        {[1,2,3,4,5].map((s) => (
          <svg key={s} className={`w-5 h-5 transition-colors duration-500 ${s <= rounded ? "text-coral-400" : "text-zinc-300 dark:text-zinc-700"}`}
      ...
      <span className="text-espresso-500 text-sm">{count > 0 ? `${count} reviews` : "No reviews yet"}</span>
    </div>
  );
}
