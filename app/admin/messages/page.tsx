...
function ReplyModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-2xl bg-zinc-900 border border-zinc-700 rounded-2xl shadow-2xl z-10">
        <div className="flex items-center justify-between p-5 border-b border-zinc-800">
          <h3 className="text-white font-semibold">Reply to {message.name}</h3>
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            ...
            {senders.length === 0 ? (
              ...
              <select
                ...
                className="w-full px-3 py-2.5 rounded-lg border border-zinc-700 bg-zinc-800 text-white text-sm focus:outline-none focus:ring-1 focus:ring-gold-500"
              >
                {senders.map((s) => (
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2.5 rounded-lg border border-zinc-700 bg-zinc-800/50 text-zinc-400 text-sm cursor-not-allowed"
            />
          </div>
          ...
          {/* Reply body */}
          <div>
            <label className="block text-xs text-zinc-400 mb-1.5">Your Reply *</label>
            <textarea
              value={body}
              onChange={(e) => setBody(e.target.value)}
              placeholder="Type your reply here..."
              rows={7}
              className="w-full px-3 py-2.5 rounded-lg border border-zinc-700 bg-zinc-800 text-white text-sm placeholder-zinc-600 focus:outline-none focus:ring-1 focus:ring-gold-500 resize-none"
            />
          </div>
        </div>
        ...
        <div className="flex gap-3 p-5 border-t border-zinc-800">
          <button
            ...
            className="flex-1 py-2.5 rounded-xl bg-gold-500 text-white text-sm font-semibold hover:bg-gold-600 disabled:opacity-60 transition-colors flex items-center justify-center gap-2"
          >
            {sending ? (
              <>
          ...
          <button
            onClick={onClose}
            className="px-5 py-2.5 rounded-xl border border-zinc-700 text-zinc-400 text-sm hover:text-white transition-colors"
          >
            Cancel
          </button>
        </div>
          ...
          {["all", "new", "read", "resolved"].map((f) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-medium capitalize transition-all ${
                filter === f ? "bg-white text-zinc-900" : "border border-zinc-700 text-zinc-400 hover:text-white"
              }`}
            >
      ...
      {loading ? (
        ...
        <div className="grid lg:grid-cols-5 gap-6">
          ...
          <div className="lg:col-span-3">
            {selected ? (
              <div className="p-6 rounded-2xl border border-zinc-800 bg-zinc-900 sticky top-6">
                ...
                <div className="space-y-4 mb-6">
                  ...
                  <div>
                    ...
                    <div className="p-4 rounded-xl bg-zinc-800 text-zinc-300 text-sm leading-relaxed">
                      {selected.message}
                    </div>
                  </div>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <button
                    ...
                    className="px-3 py-2 rounded-lg border border-zinc-700 text-zinc-400 text-sm hover:text-white disabled:opacity-40 transition-colors"
                  >
                    Mark Read
                  </button>
                  <button
              ...
              <div className="p-8 rounded-2xl border border-dashed border-zinc-800 flex items-center justify-center text-zinc-600 text-sm h-64">
