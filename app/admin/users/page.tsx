...
export default function AdminUsers() {
 ...
 <h1 className="font-display text-3xl font-black text-white leading-none">Users</h1>
 ...
 <div className="p-4 border border-amber-800/30 bg-amber-900/10 mb-6">
 ...
 {showForm && (
 <form onSubmit={handleAdd} className="mb-6 p-6 border border-zinc-800 bg-zinc-900">
 <h2 className="font-display text-lg font-bold text-white mb-4">New Admin User</h2>
 <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
 <div>
 <label className="block text-[10px] font-bold tracking-[0.2em] uppercase text-zinc-500 mb-2">Email Address</label>
 ...
 <div>
 <label className="block text-[10px] font-bold tracking-[0.2em] uppercase text-zinc-500 mb-2">Password (min. 8 chars)</label>
 ...
 {loading ? (
 <div className="space-y-3">{[...Array(3)].map((_,i)=><div key={i} className="h-16 bg-zinc-800/60 animate-pulse"/>)}</div>
 ) : users.length === 0 ? (
 <p className="text-center py-16 text-zinc-600">No users found.</p>
 ) : (
 <div className="space-y-3">
 {users.map((user) => (
 <div key={user.id} className="flex items-center justify-between p-5 border border-zinc-800 bg-[#0c0c0c] hover:border-zinc-800 transition-colors">
 ...
 className="px-3 py-1.5 border border-zinc-800 text-zinc-400 text-xs hover:text-white hover:border-zinc-500 transition-colors disabled:opacity-50">
 {resetting===user.email ? "Sending..." : "Reset Password"}
 </button>
 <button onClick={()=>setConfirmState({type:'delete', id:user.id, email:user.email})}
 className="px-3 py-1.5 bg-red-500/15 text-red-400 text-xs hover:bg-red-500/25 transition-colors">
 Delete
 </button>
 </div>
 </div>
