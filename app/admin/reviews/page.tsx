...
export default function AdminReviews() {
 ...
 filter === f.key ? "bg-gold-500 text-white" : "border border-zinc-800/60 text-zinc-500 hover:text-white"
 }`}
 >
 {f.label}
 </button>
 ...
 {/* Info banner */}
 <div className="p-4 border border-blue-800/30 bg-blue-900/10 mb-6 flex items-start gap-3">
 ...
 {loading ? (
 <div className="space-y-3">{[...Array(4)].map((_, i) => <div key={i} className="h-24 bg-zinc-800/60 animate-pulse" />)}</div>
 ) : filtered.length === 0 ? (
 ...
 : "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 hover:bg-emerald-500/30"
 }`}
 >
 ...
 className="px-3 py-1.5 bg-red-500/20 text-red-400 text-xs font-medium hover:bg-red-500/30 transition-colors"
 >
 Delete
 </button>
 </div>
