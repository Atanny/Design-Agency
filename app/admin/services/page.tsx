    ...
    <div className="p-8 w-full max-w-full">
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {[...Array(6)].map((_, i) => <div key={i} className="h-48 rounded-2xl bg-zinc-800 animate-pulse" />)}
        </div>
      ) : services.length === 0 ? (
            ...
            >
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => openEdit(s)}
                  className="flex-1 py-2 rounded-lg border border-zinc-700 text-zinc-300 text-xs font-semibold hover:bg-zinc-800 transition-colors"
                >Edit</button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-5">
              {/* Title */}
              <div>
                <label className="block text-xs font-medium text-zinc-400 mb-1.5">Service Title *</label>
              ...
              {/* Features */}
              <div>
                <label className="block text-xs font-medium text-zinc-400 mb-1.5">Features (one per line — shown on Services page)</label>
                  ...
                  {ICON_OPTIONS.map((ico) => (
                    <button key={ico.value} onClick={() => setEditing({ ...editing, icon: ico.value })}
                      className={`flex flex-col items-center gap-1.5 p-3 rounded-xl border transition-all ${
                        editing.icon === ico.value ? "border-gold-500 bg-gold-500/10 text-gold-400" : "border-zinc-700 text-zinc-500 hover:border-zinc-600"
                      }`}
                    >
                      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div className="flex flex-wrap gap-2">
                  {ACCENT_OPTIONS.map((acc) => (
                    <button key={acc.value} onClick={() => setEditing({ ...editing, accent: acc.value, bg_color: acc.bg })}
                      className={`px-4 py-2 rounded-lg text-sm font-medium border transition-all ${acc.bg} ${acc.value} ${
            ...
            <div className="flex gap-3 p-6 border-t border-zinc-800">
              <button onClick={handleSave} disabled={saving}
                className="flex-1 py-3 rounded-xl bg-gold-500 text-white text-sm font-semibold hover:bg-gold-600 disabled:opacity-50 transition-colors"
              >
