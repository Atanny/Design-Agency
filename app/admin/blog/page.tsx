...
const emptyForm = (): Partial<BlogPost> => ({ title:"", slug:"", content:"", featured_image:"", meta_title:"", meta_description:"", published:false });
...
export default function AdminBlog() {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const { error, data } = await supabase.storage.from("blog").upload(`blog-${Date.now()}.${ext}`, file, {upsert:true});
    if (error) { toast.error("Upload failed"); }
    else { const { data: url } = supabase.storage.from("blog").getPublicUrl(data.path); setForm(f=>({...f,featured_image:url.publicUrl})); toast.success("Image uploaded!"); }
    setUploading(false);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setMode("list"); setForm(emptyForm()); fetchPosts();
  };
    ...
    <div className="p-6 w-full">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-12 gap-3 mb-4">
        <div className="sm:col-span-8 rounded-2xl bg-zinc-900 border border-zinc-800/50 p-6 flex flex-col justify-between min-h-[110px]">
          ...
          <div>
            ...
            <p className="text-zinc-500 text-sm mt-1 font-light">{posts.length} post{posts.length!==1?"s":""}</p>
          </div>
        </div>
      ...
      {loading ? (
        <div className="grid grid-cols-1 gap-3">
          {[...Array(4)].map((_,i)=><div key={i} className="h-20 rounded-2xl bg-zinc-900 animate-pulse"/>)}
        </div>
      ) : posts.length === 0 ? (
        <div className="rounded-2xl bg-zinc-900 border border-zinc-800/50 p-16 text-center text-zinc-600">
          No blog posts yet. Create your first post!
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-3">
          {posts.map(post=>(
            <div key={post.id} className="rounded-2xl bg-zinc-900 border border-zinc-800/50 p-5 flex items-center justify-between hover:border-zinc-700 transition-all">
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                  <p className="text-white font-medium truncate">{post.title}</p>
                  {post.published
                    ? <span className="px-2.5 py-1 rounded-full bg-emerald-500/10 text-emerald-400 text-[10px] font-semibold border border-emerald-500/20 flex-shrink-0">Published</span>
                    : <span className="px-2.5 py-1 rounded-full bg-zinc-800 text-zinc-500 text-[10px] font-semibold border border-zinc-700/60 flex-shrink-0">Draft</span>
                  }
                </div>
              ...
              <div className="flex gap-2 flex-shrink-0 ml-4">
                <button onClick={()=>{ setForm(post); setMode("edit"); }}
                  className="px-3 py-1.5 rounded-lg border border-zinc-800/60 text-zinc-400 text-xs font-medium hover:text-white hover:border-zinc-600 transition-colors">
                  Edit
                </button>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm">
          <div className="rounded-2xl bg-zinc-900 border border-zinc-800/50 w-full max-w-3xl max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <form onSubmit={handleSave} className="p-6 space-y-5">
              ...
              <div>
                ...
                <input type="text" value={form.title||""} placeholder="Post title"
                  onChange={e=>{const title=e.target.value;setForm(f=>({...f,title,slug:f.slug||slugify(title),meta_title:f.meta_title||title}));}}
              ...
              {/* Slug */}
              <div>
                <label className="block text-[10px] font-semibold tracking-[0.2em] uppercase text-zinc-500 mb-2">Slug *</label>
                <input type="text" value={form.slug||""} placeholder="post-slug"
              ...
              {/* Featured image */}
              <div>
                <label className="block text-[10px] font-semibold tracking-[0.2em] uppercase text-zinc-500 mb-2">Featured Image</label>
                <input ref={fileRef} type="file" accept="image/*" onChange={handleImageUpload} className="hidden"/>
                {form.featured_image ? (
                  <div className="flex items-center gap-4">
                    <img src={form.featured_image} alt="Featured" className="w-24 h-16 object-cover rounded-xl border border-zinc-800/50 flex-shrink-0"/>
                    <div className="flex gap-2">
                      <button type="button" onClick={()=>fileRef.current?.click()} disabled={uploading}
                        className="px-3 py-2 rounded-lg border border-zinc-800/60 text-zinc-400 text-xs hover:text-white disabled:opacity-50 transition-colors">
                  ...
                  <div className="flex gap-3">
                    <input type="text" value={form.featured_image||""} placeholder="Image URL or upload..."
              ...
              {/* Meta description */}
              <div>
                <label className="block text-[10px] font-semibold tracking-[0.2em] uppercase text-zinc-500 mb-2">Meta Description</label>
                <textarea value={form.meta_description||""} placeholder="SEO description (150-160 chars)..." rows={2} maxLength={160}
              ...
              {/* Content */}
              <div>
                <label className="block text-[10px] font-semibold tracking-[0.2em] uppercase text-zinc-500 mb-2">Content (HTML)</label>
                <textarea value={form.content||""} placeholder="<p>Post content in HTML...</p>" rows={16}
              ...
              <div className="flex gap-3 pt-2">
                <button type="submit" className="px-6 py-3 gradient-primary text-white rounded-xl text-sm font-bold hover:opacity-90 transition-colors">
