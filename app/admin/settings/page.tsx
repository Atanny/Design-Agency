...
export default function AdminSettings() {
 ...
 <div className="p-6 border border-amber-800/30 bg-amber-900/10">
 ...
 <div>
 <h2 className="font-display text-lg font-bold text-white">Email Senders</h2>
 <p className="text-zinc-500 text-xs mt-0.5">Shown in the From dropdown when replying to messages.</p>
 </div>
 <button onClick={() => setShowAddForm(!showAddForm)}
 className="flex items-center gap-1.5 px-3 py-2 bg-gold-500/20 text-gold-400 text-sm font-medium hover:bg-gold-500/30 transition-colors">
 ...
 {showAddForm && (
 <form onSubmit={handleAdd} className="mb-5 p-4 bg-zinc-800 border border-zinc-800/60">
 <div className="grid grid-cols-2 gap-3 mb-3">
 <div>
 <label className="block text-[10px] font-bold tracking-[0.15em] uppercase text-zinc-600 mb-1.5">Display Name</label>
 <input type="text" value={addForm.name} onChange={(e)=>setAddForm(f=>({...f,name:e.target.value}))} placeholder="Your Studio Name"
 className="w-full px-3 py-2 border border-zinc-800/60 bg-zinc-900 text-white text-sm placeholder-zinc-700 focus:outline-none focus:ring-1 focus:border-gold-500/50 focus:ring-0"/>
 </div>
 <div>
 <label className="block text-[10px] font-bold tracking-[0.15em] uppercase text-zinc-600 mb-1.5">Email Address</label>
 ...
 className="px-4 py-2 border border-zinc-800/60 text-zinc-500 text-sm font-semibold hover:text-white hover:border-zinc-600 transition-all">
 Cancel
 </button>
 </div>
 </form>
 ...
 {loading ? (
 <div className="space-y-2">{[...Array(2)].map((_,i)=><div key={i} className="h-14 bg-zinc-900 animate-pulse"/>)}</div>
 ) : senders.length === 0 ? (
 ...
 className="px-3 py-1.5 border border-zinc-800/60 text-zinc-500 text-xs font-semibold hover:text-white transition-all">
 Set Default
 </button>
 )}
 <button onClick={()=>setConfirmDelete(sender.id)}
 className="px-3 py-1.5 bg-red-500/20 text-red-400 text-xs hover:bg-red-500/30 transition-colors">
 Remove
 </button>
 </div>
 </div>
 ...
 <div className="p-6 border border-zinc-800 bg-[#0c0c0c] h-fit">
 <h3 className="font-display text-lg font-bold text-white mb-5">Required Environment Variables</h3>
 <div className="space-y-3">
 {ENV_VARS.map((env) => (
 <div key={env.key} className="flex items-start gap-3 p-3 bg-zinc-900/50 border border-zinc-800/60">
 <code className="text-xs bg-[#0c0c0c] text-gold-300 px-2 py-1 rounded font-mono flex-shrink-0 mt-0.5">{env.key}</code>
