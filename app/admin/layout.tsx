...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    fetchLogo();
    const channel = supabase.channel("admin-logo-watch")
      ...
      <aside className="w-60 bg-[#0a0a0a] flex flex-col fixed inset-y-0 left-0 z-30 p-3 gap-3">
        ...
        <div className="rounded-2xl bg-zinc-900 border border-zinc-800/50 p-3">
          ...
          <button onClick={handleSignOut}
            className="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-zinc-600 hover:text-red-400 transition-colors group"
          >
