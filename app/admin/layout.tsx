...
const navItems = [
  ...
  { href: "/admin/content",   label: "Content",   icon: "edit2" },
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const Sidebar = () => (
    <aside className="w-60 bg-zinc-900 border-r border-zinc-800 flex flex-col h-full">
      ...
      <nav className="flex-1 p-3 space-y-0.5 overflow-y-auto">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <span className={active ? "text-gold-400" : ""}><Icon name={item.icon} /></span>
              {item.label}
            </Link>
          );
        })}
  ...
  return (
    <div className="min-h-screen bg-zinc-950 flex">
      ...
      <div className="flex-1 lg:ml-60 flex flex-col min-h-screen w-full min-w-0">
        {/* Mobile topbar */}
        <div className="lg:hidden flex items-center gap-3 px-4 py-3 bg-zinc-900 border-b border-zinc-800 sticky top-0 z-40">
          <button
          ...
          <span className="font-display text-white font-semibold text-sm">Lumis Studio</span>
        </div>
