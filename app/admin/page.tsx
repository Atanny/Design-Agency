...
function RatingDonut({ distribution }: { distribution: number[] }) {
  ...
  const slices = distribution.map((count, i) => {
    ...
    cumulative += pct;
    return { pct, start, color: colors[i] };
  }).filter(s => s.pct > 0);
      ...
      <div className="relative w-24 h-24 flex-shrink-0">
        <svg viewBox="0 0 100 100" className="w-full h-full">
          {slices.length === 0
            ...
            : slices.map((s, i) => <path key={i} d={describeArc(s.start, s.start + s.pct)} fill={s.color} opacity="0.85" />)
          }
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [port,rev,revPend,msg,msgNew,blog,blogPub,svc,recentMsg,recentRev,allMsgs,allRevs,commData] =
        await Promise.all([
      ...
      setRecent({
        messages:(recentMsg.data as RecentMessage[])||[],
        reviews:(recentRev.data as RecentReview[])||[],
      });

      // Build 6-month message chart
      ...
      (allMsgs.data || []).forEach((m: { created_at: string }) => {
        ...
        if (idx >= 0 && idx < 6) months[idx].value++;
      });
      setMsgChart(months);
      ...
      if (commData.data) {
        ...
        commData.data.forEach((r: { key:string; value:string }) => { map[r.key] = r.value; });
  ...
  const cards = [
    { label:"Portfolio",      value:stats.portfolio,    href:"/admin/portfolio",  color:"bg-blue-500/10 text-blue-400",    sub:null,
    ...
    { label:"Messages",       value:stats.messages,     href:"/admin/messages",   color:"bg-emerald-500/10 text-emerald-400", sub:stats.messagesNew > 0 ? `${stats.messagesNew} new` : null,
      icon:<path strokeLinecap="round" strokeLinejoin="round" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/> },
    { label:"Reviews",        value:stats.reviews,      href:"/admin/reviews",    color:"bg-amber-500/10 text-amber-400",   sub:stats.reviewsPending > 0 ? `${stats.reviewsPending} pending` : null,
    ...
    { label:"Blog Posts",     value:stats.blog,         href:"/admin/blog",       color:"bg-violet-500/10 text-violet-400", sub:stats.blogPublished > 0 ? `${stats.blogPublished} live` : null,
    ...
    { label:"Services",       value:stats.services,     href:"/admin/services",   color:"bg-coral-400/10 text-coral-400",  sub:null,
    ...
    <div className="p-6 w-full max-w-full">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-12 gap-3 mb-3">
        <div className="sm:col-span-8 rounded-2xl bg-zinc-900 border border-zinc-800/50 p-6 flex flex-col justify-between min-h-[110px]">
          <p className="text-[10px] font-semibold tracking-[0.22em] uppercase text-amber-500">Admin Dashboard</p>
          <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 mb-3">
          {cards.map((c) => (
            <Link key={c.label} href={c.href}
              className="group rounded-2xl bg-zinc-900 border border-zinc-800/50 p-5 hover:border-zinc-700 transition-all flex flex-col justify-between min-h-[110px]">
              ...
              <div>
                ...
                {c.sub && <p className="text-[11px] text-amber-400 mt-0.5 font-semibold">{c.sub}</p>}
              </div>
            </Link>
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 mb-3">
        <div className="lg:col-span-2 rounded-2xl bg-zinc-900 border border-zinc-800/50 p-6">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h2 className="text-[11px] font-semibold tracking-[0.2em] uppercase text-amber-500">Messages</h2>
          </div>
          ...
          {loading
            ...
            : <MiniBar data={msgChart} color="bg-amber-500"/>
          }
        </div>
        <div className="rounded-2xl bg-zinc-900 border border-zinc-800/50 p-6">
          <div className="flex items-center gap-2 mb-1">
            <div className="h-px w-4 bg-amber-500/50"/>
            <h2 className="text-[11px] font-semibold tracking-[0.2em] uppercase text-amber-500">Ratings</h2>
          </div>
          ...
          {loading
            ...
            : <RatingDonut distribution={ratingDist}/>
          }
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 mb-3">
        <div className="rounded-2xl bg-zinc-900 border border-zinc-800/50 overflow-hidden">
          <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800/40">
            ...
            <Link href="/admin/messages" className="text-xs text-amber-400 hover:text-amber-300 transition-colors font-medium">View all →</Link>
          </div>
          {loading
        ...
        <div className="rounded-2xl bg-zinc-900 border border-zinc-800/50 overflow-hidden">
          <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800/40">
            ...
            <Link href="/admin/reviews" className="text-xs text-amber-400 hover:text-amber-300 transition-colors font-medium">View all →</Link>
          </div>
          {loading
            ? <div className="p-5 space-y-3">{[...Array(4)].map((_,i)=><div key={i} className="h-10 rounded-xl bg-zinc-800 animate-pulse"/>)}</div>
                    ...
                    <div key={r.id} className="flex items-center justify-between px-6 py-3.5 hover:bg-zinc-800/30 transition-colors">
                      <div className="min-w-0">
                        ...
                        <div className="flex gap-0.5 mt-0.5">
                          {[1,2,3,4,5].map((s) => (
                            <svg key={s} className={`w-3 h-3 ${s<=r.rating ? "text-amber-400" : "text-zinc-700"}`} fill="currentColor" viewBox="0 0 20 20">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            { label:"Upload Portfolio", href:"/admin/portfolio", color:"rounded-2xl bg-blue-500/10 border border-blue-500/20 text-blue-400 hover:bg-blue-500/15" },
            { label:"Write Blog Post",  href:"/admin/blog",      color:"rounded-2xl bg-violet-500/10 border border-violet-500/20 text-violet-400 hover:bg-violet-500/15" },
            { label:"View Messages",    href:"/admin/messages",  color:"rounded-2xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 hover:bg-emerald-500/15" },
            { label:"Approve Reviews",  href:"/admin/reviews",   color:"rounded-2xl bg-amber-500/10 border border-amber-500/20 text-amber-400 hover:bg-amber-500/15" },
          ].map((a) => (
            <Link key={a.label} href={a.href} className={`p-5 text-sm font-medium transition-all hover:scale-[1.02] ${a.color}`}>
              {a.label} →
            </Link>
          ))}
        </div>
