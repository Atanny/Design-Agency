      ...
      {open && (
        <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
          ...
          <motion.div initial={{scale:0.9,opacity:0,y:16}} animate={{scale:1,opacity:1,y:0}} exit={{scale:0.92,opacity:0}}
            ...
            className="relative w-full max-w-sm bento-card bg-card rounded-3xl shadow-2xl overflow-hidden"
            onClick={e=>e.stopPropagation()}>
            <div className="h-1.5 gradient-primary w-full"/>
            <div className="p-6">
              <div className="text-3xl mb-3">{icon}</div>
              <h3 className="font-display text-xl font-bold text-page mb-2">{title}</h3>
              <p className="text-body text-sm leading-relaxed mb-1">{message}</p>
              ...
              <div className="flex gap-3">
                ...
                <button onClick={onCancel} className="flex-1 py-2.5 rounded-full text-sm font-semibold border-2 border-card text-body hover:border-coral-300 transition-all">{cancelLabel}</button>
              </div>
            </div>
          </motion.div>
        </motion.div>
