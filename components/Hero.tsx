...
export default function Hero({ content = {} }: HeroProps) {
    ...
    async function fetchStats() {
      try {
        ...
        const avg = reviewList.length
          ...
          : 0;
        setProjects(portfolioRes.count ?? 0);
  ...
  const cta = content.cta_button || "Contact Us Now";

  const stats = [
    {
      value: projects === null ? "..." : projects > 0 ? `${projects}+` : "0",
      label: content.stat1_label || "Projects Completed",
    },
    {
      value: reviewCount === null ? "..." : reviewCount > 0
        ? `${Math.min(99, Math.round(80 + (reviewCount / Math.max(reviewCount, 10)) * 19))}%`
        : "0%",
      label: content.stat2_label || "Client Satisfaction",
    },
    {
      value: avgRating === null ? "..." : avgRating > 0 ? `${avgRating}★` : "0★",
  ...
  return (
    <section className="relative min-h-screen flex flex-col justify-center overflow-hidden pt-24 pb-16">
          ...
          >
            {stats.map((stat) => (
              <div key={stat.label} className="text-center">
                <div className="font-display text-3xl md:text-4xl font-bold text-zinc-900 dark:text-white mb-1">
                  {stat.value}
                </div>
      ...
      {/* Scroll indicator - separated from stats, no overlap */}
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.2 }}
        className="flex flex-col items-center gap-2 text-zinc-400 mt-10 pb-4"
      >
